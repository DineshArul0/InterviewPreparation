/**
 * Script to directly update java-easy.json to have 100 questions
 */

const fs = require('fs');
const path = require('path');

// Create the complete JSON structure with all 100 questions
const completeData = {
  "id": "java-easy",
  "name": "Java - Easy",
...
// Fill in the questions (1-32 are already in the file)
// We'll define these questions directly in the script

// Questions 1-32 (already in the file)
// Questions 33-100 (to be added)
...
const questions = [
  ...
  // Questions 3-32 are already in the file
  // ... (skipping to save space)
  
  // Start adding from question 33
  {
    "id": "java-easy-033",
    "type": "theory",
    "difficulty": "Easy",
...
// Read the current file to get the first 32 questions
try {
  const jsonFilePath = path.join(__dirname, '..', 'data', 'java-easy.json');
  const currentData = JSON.parse(fs.readFileSync(jsonFilePath, 'utf8'));
  
  // Get the first 32 questions
  const existingQuestions = currentData.questions;
  
  // Add the remaining questions (33-100)
  const remainingQuestions = getQuestionsFrom33To100();
  
  // Combine the questions
  completeData.questions = [...existingQuestions, ...remainingQuestions];
  
  // Write the updated JSON to the file
  fs.writeFileSync(jsonFilePath, JSON.stringify(completeData, null, 2));
  console.log(`Successfully updated java-easy.json with ${completeData.questions.length} questions`);
} catch (error) {
  console.error('Error updating the file:', error);
}

// Function to return all questions from 33 to 100
function getQuestionsFrom33To100() {
  return [
    {
      "id": "java-easy-033",
      "type": "theory",
